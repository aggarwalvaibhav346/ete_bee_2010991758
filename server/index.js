...
app.post("/signup/insert-data",(req,res)=>{
	...
	const pass = bcrypt.hashSync(password, saltfuck);
    var postAll="";
    if(check==="buyer"){
...
app.get("/login/check-data",(req,res)=>{
    ...
    if(check==="buyer")
    {
        var postAll="select * from profile where email=? and buyer=?;"
    }
    else
    {
        var postAll="select * from profile where email=? and dealer=?;"
    }
    db.query(postAll,[email,1],(err,result)=>{
        try{
